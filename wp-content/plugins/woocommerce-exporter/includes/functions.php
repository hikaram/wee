...
if( is_admin() ) {
	...
	function woo_ce_fail_notices() {
		...
		if( isset( $_GET['failed'] ) ) {
			...
			if( $message )
				...
				$message = __( 'A WordPress or server error caused the exporter to fail, no reason was provided, please get in touch so we can reproduce and resolve this.', 'woocommerce-exporter' ) . ' (<a href="' . $troubleshooting_url . '" target="_blank">' . __( 'Need help?', 'woocommerce-exporter' ) . '</a>)';
	...
	function woo_ce_save_fields( $export_type = '', $fields = array(), $sorting = array() ) {
		...
		if( $fields == false && !is_array( $fields ) )
			$fields = array();
		$export_types = array_keys( woo_ce_return_export_types() );

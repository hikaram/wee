...
class Vc_Role_Access_Controller extends Vc_Access {
	...
	public function getState() {
		$state = isset( $this->getRole()->capabilities[ $this->getStateKey() ] ) ?
			$this->getRole()->capabilities[ $this->getStateKey() ] : null;
	...
	public function can( $rule = '', $check_state = true ) {
		if ( null === $this->getRole() ) {
			...
			if ( 'administrator' === $this->getRole()->name && 'settings' === $this->getPart() &&
			     ( 'vc-roles-tab' === $rule || 'vc-updater-tab' === $rule )
			) {
			...
			$return = apply_filters( 'vc_role_access_with_' . $this->getPart() . '_can',
			$return, $this->getRole(), $rule );
			$return = apply_filters( 'vc_role_access_with_' . $this->getPart() . '_can_' . $rule,
			$return, $this->getRole() );
	...
	public function getCapRule( $rule ) {
		...
		return $this->getRole()->has_cap( $rule );
	}
	...
	public function setCapRule( $rule, $value = true ) {
		...
		$this->getRole()->add_cap( $role_rule, $value );
	}

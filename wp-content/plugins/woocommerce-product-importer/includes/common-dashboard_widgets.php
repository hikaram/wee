...
if( !function_exists( 'woo_vl_dashboard_setup' ) ) {
	...
	function woo_vl_dashboard_setup() {
		...
		if( current_user_can( 'manage_options' ) ) {
			wp_add_dashboard_widget( 'woo_vl_news_widget', __( 'Plugin News - by Visser Labs', 'woo_pi' ), 'woo_vl_news_widget' );
		}
	...
	function woo_vl_news_widget() {
		...
		if( !is_wp_error( $rss ) ) {
			...
			$message = __( 'Connection failed. Please check your network settings.', 'woo_pi' );
			$output .= '<p>' . $message . '</p>';
		}
		$output .= '</div>';

<?php
    ...
    $colors = array("red", "green", "orange", "blue", "blue2", "gray"); $colorindex = 0;
    if ($all_categories) {
...
<div class="category-featured">
    ...
    if ($colorindex >= sizeof($colors)) $colorindex = 0; print($colors[$colorindex++]); ?> show-brand">
      <div class="container">
        <!-- Brand and toggle get grouped for better mobile display -->
          <div class="navbar-brand"><a href="#"><img alt="fashion" src=<?php print("\"".$catimagelink."\""); ?> /><?php echo $cat->name; ?></a></div>
...
<?php
	...
	?>
            <li<?php if ($bFirst) { $bFirst = FALSE; print(" class=\"active\""); } ?>><a data-toggle="tab" href=<?php print("\"#tab-".$sc->term_id."\""); ?>><?php print($sc->name); ?></a></li>
<?php $iSCPos++; } ?>
          </ul>
<?php if($iSCPos >= 4) {
...
?>
                    <div class="tab-panel<?php if ($bFirst) { $bFirst = FALSE; print(" active"); } ?>" id="tab-<?php print($sc_id); ?>">
...
//  $args1 = array( 'post_type' => 'product', 'posts_per_page' => 10, 'product_cat' => $cat->term_id, 'orderby' => 'rand' );
  ...
  while ( $loop->have_posts() ) : $loop->the_post(); global $product; ?>
				...
				?>
                                    ...
                                    <img class="img-responsive" style="height: 199px; " alt="<?php echo esc_attr($loop->post->post_title); ?>" src="<?php
                                	echo $image_link;
                            	    ?>" /></a>
                                    ...
                                    <div class="add-to-cart">
                                        <a title="Добавить в корзину" data-product_id="<?php echo $loop->post->ID; ?>" class="button add_to_cart_button"  href="/?add-to-cart=<?php echo $loop->post->ID; ?>">Добавить в корзину</a>
                                    </div>
                                </div>
                                <div class="right-block">
                                    <h5 class="product-name"><a href="<?php echo get_permalink( $loop->post->ID ) ?>"><?php echo esc_attr($loop->post->post_title); ?></a></h5>
                                    <div class="content_price">
                                        <del>
                                        <span class="amount"><?php print($price."&nbsp;руб."); ?></span>
                                        </del>
                                        <ins style="text-decoration:none;">
                                        <span class="amount"><?php print($sale."&nbsp;руб."); ?></span>
                                        </ins>
                                    </div>
